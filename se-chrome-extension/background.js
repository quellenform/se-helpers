...
function executeScriptDestroyUser() {
    getActiveTab((tab) => {
        chrome.scripting.executeScript({
            target: { tabId: tab.id },
            func: () => {
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    ...
    switch (message.action) {
        case 'navigate':
            ...
            break;
        case 'notify':
            handleNotification(message.title, message.message);
            break;
        case 'executeScriptDestroyUser':
            executeScriptDestroyUser();
            break;
        case 'executeScriptForSpam':
            executeScriptForSpam();
            break;
        default:
            console.warn('SE Helper: Unknown action:', message.action);
    }
});
